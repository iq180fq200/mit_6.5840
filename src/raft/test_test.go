...
func TestBasicAgree2B(t *testing.T) {
	...
	for index := 1; index < iters+1; index++ {
		...
		if xindex != index {
			...
			Debug(dClient, "command %d succeed", index*100)
		}
	}
...
func TestBackup2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	Debug(dInstruct, "%d is leader", leader1)
	cfg.disconnect((leader1 + 2) % servers)
	Debug(dInstruct, "%d disconectted", (leader1+2)%servers)
	cfg.disconnect((leader1 + 3) % servers)
	Debug(dInstruct, "%d disconectted", (leader1+3)%servers)
	cfg.disconnect((leader1 + 4) % servers)
	Debug(dInstruct, "%d disconectted", (leader1+4)%servers)

	// submit lots of commands that won't commit
	for i := 0; i < 50; i++ {
		cfg.rafts[leader1].Start(rand.Int())
	}
	Debug(dInstruct, "!!!submit lots of commands that won't commit done")

	time.Sleep(RaftElectionTimeout / 2)

	cfg.disconnect((leader1 + 0) % servers)
	Debug(dInstruct, "%d disconectted", (leader1+0)%servers)
	cfg.disconnect((leader1 + 1) % servers)
	Debug(dInstruct, "%d disconectted", (leader1+1)%servers)
	...
	Debug(dInstruct, "!!!hint: other three recover done, and first two die")

	// lots of successful commands to new group.
	for i := 0; i < 50; i++ {
		cfg.one(rand.Int(), 3, true)
	}
	Debug(dInstruct, "!!!lots of successful commands to new group done")
	...
	Debug(dInstruct, "!!!hint: a follower disconnect")
	Debug_ = 1
	...
	time.Sleep(RaftElectionTimeout / 2)
	Debug(dInstruct, "!!!lots more commands that won't commit done")
	...
	cfg.connect(other)
	Debug(dInstruct, "!!!bring original leader back to life done, the disconnected follower in last step connected")
	...
	Debug(dInstruct, "!!!bring back everyone")
	cfg.one(rand.Int(), servers, true)
	Debug(dInstruct, " !!!everyone done")
